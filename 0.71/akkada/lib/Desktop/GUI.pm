...
sub prepare_left
{
    ...
    {
        $left->content( $self->find->form_entity_find );
    }
    else
    {
        return;
...
sub server_time
{
    return sprintf("server time: %s", scalar localtime);
}
...
sub prepare_right_contacts
{
    ...
    $right->content( $contacts->contacts_cgroups_table() );
}
...
sub prepare_right_permissions
{
    ...
    {
        $right->content( $perm->users_and_groups_table() );
    }
    elsif ($url_params->{mode} == 2)
    {
        $right->content( $perm->rights_table() );
    }
}
...
sub tool_get
{
    ...
    if (! defined $url_params->{start} || ! $url_params->{start})
    {
...
sub prepare_right_entity
{
    ...
    $tab->add(
    { 
        caption => "services options" . tip(13), 
        class => 'v', 
        active => $url_params->{section}, 
    ...
    elsif ( $url_params->{section} eq 'alarms') 
    {
        ...
        $url_params->{correlation} 
            ? $but->add({ 
                ...
                class => 'p2',  url => sprintf(qq|%s?%s|, url_get({}, $url_params), 'form_name=form_correlation'), })
            : $but->add({ 
...
#DUPA
    ...
    elsif ( $url_params->{section} eq 'history' && $self->matrix('history', $node))
    {
        $but->add(
        {
            caption => '' . tip(7) , alt => 'clear history filter', img => '/img/log_filter_clear.gif',
            class => 'p2', 
            url => url_get({ id_entity => $id_entity, 'clear_history_filter' => 1, }, $url_params ), 
        })
            if session_get_param($self->session, '_HISTORY_CONDITIONS')
                && $self->matrix('form_history_filter', $node);

        ($url_params->{'show_form_history_filter'} 
            ? $but->add(
            {
                 caption => '' . tip(6) , alt => 'hide history filter', img => '/img/log_filter_off.gif',
                 class => 'p2', 
                 url => url_get({ id_entity => $id_entity, 'show_form_history_filter' => 0, }, $url_params ),
            })
            : $but->add(
            {
                 caption => '' . tip(5) , alt => 'show history filter', img => '/img/log_filter_on.gif',
                 class => 'p2',
                 url => url_get({ id_entity => $id_entity, 'show_form_history_filter' => 1, }, $url_params ), 
            }))
        ...
        $content->addRow( $entity_cgi->form_history_filter() )
            if $url_params->{'show_form_history_filter'}
                && $self->matrix('form_history_filter', $node);
    ...
    {
        my $local_permition = $self->matrix('form_entity_discover', $node);
        ...
        if ($url_params->{what_op} eq '1' && $local_permition)
        {
    ...
    elsif ( $url_params->{section} eq 'stat' && $self->matrix('stat', $node))
    {
        ...
        if ($node->id_probe_type eq '1' && ! defined $VIEWS_FIND{$view_mode})
        {
            ! session_get_param($self->session, '_STAT_SHOW_NODE_INFO')
                ? $but->add(
                {
                    caption => 'show node info', alt => 'show node information', img => '/img/node_info_on.gif',
                    url => sprintf(qq|%s?%s|, url_get({}, $url_params), 'form_name=form_stat_show_node_info'), 
                })
                : $but->add(
                {
                    caption => 'hide node info', alt => 'hide node information', img => '/img/node_info_off.gif',
                    url => sprintf(qq|%s?%s|, url_get({}, $url_params), 'form_name=form_stat_show_node_info'), 
                })
                if $self->matrix('form_stat_show_node_info', $node);
        }

        session_get_param($self->session, '_STAT_SHOW_GRAPH_OPTIONS')
            ? $but->add(
            {
                caption => '', alt => 'hide graph options', img => '/img/stat_params_off.gif',
                url => sprintf(qq|%s?%s|, url_get({}, $url_params), 'form_name=form_stat_show_graph_options'), 
            })
            : $but->add(
            {
                caption => '', alt => 'show graph options', img => '/img/stat_params_on.gif',
                url => sprintf(qq|%s?%s|, url_get({}, $url_params), 'form_name=form_stat_show_graph_options'), 
            })
        ...
        $content->addRow( $entity_cgi->form_graph_options() )
            if $self->matrix('form_graph_options', $node)
                && session_get_param($self->session, '_STAT_SHOW_GRAPH_OPTIONS');
    ...
    {
        $popups .= $probes->{'group'}->popup_menu_graphtime();
    }
...
sub prepare_form_processor
{
    ...
    if ($url_params->{form}->{form_name} eq 'form_view_select'
        ...
        || $url_params->{form}->{form_name} eq 'form_find_entity'
...
sub get_caption
{
        ...
        while (@caption)
        {
            ...
            push @result, $entity_cgi->a_popup(
            {
                ...
                name => $caption[$#caption]->[1]
                    ? ($caption[$#caption]->[1] eq 'root' ? 'location' : $caption[$#caption]->[1]) 
                    : 'unknown',
                class => '',
    ...
    if (defined $VIEWS_TREEFIND{$view_mode}) 
    {
       ...
       $id_parent = $self->session->param('_FIND')->{id_parent};
       if ($probe && $id_parent) {
    ...
    if (defined $VIEWS_LIGHT{$view_mode}) 
    {
        ...
        return 
            ...
            . join(" > ", @result) 
            . sprintf(qq|<font class="f"><b> :: %s</b></font>|, 
                $self->make_section_button($entity_cgi, $id, $last_probe_id, $start)) 
            . qq|</td></tr></table>|;
    }
    else
    {
...
sub prepare_right
{
    ...
    if ($url_params->{clear_graph_options})
    {
...
sub copyrights
{
    return '© 2005-2007 <a href="mailto:piotr.kodzis@inteligo.pl">k&#248;dz&#237;s</a>';
}
...
sub prepare_top
{
    ...
    $buttons->add(
    {
        caption => ($url_params->{section} ne 'permissions' 
                    ...
                    ? "<b>network</b>" : "network") . tip(16), 
        class => 'p2',
        url => url_get({ section => 'general', }, {}), 
    });
...
EOF
    ...
    if (defined $VIEWS_HARD{$view_mode})
    {
        $result .= <<EOF;
