...
use constant
{
    ...
    DATA => 17,
    DESCRIPTION_STATIC => 18,
    DESCRIPTION_DYNAMIC => 19,
        ...
        if ($self->[STATUS] eq $_[0])
        {
            if ($self->flap)
            {
                ...
                if ($delta > 0)
                {
                    ...
                    if ($delta > 16*$FlapsTimeSlot)
                    {
#print "KONIEC FLAPOWANIA!!!";
#print "ENTITY: ", $self->[ID_ENTITY], "\n";
#print "BEFORE: ", $self->[FLAP_MONITOR], "\n";
                        $self->flap_stop;
#print "AFTER:  ", $self->[FLAP_MONITOR], "\n";
    ...
    $self->dbh->exec(sprintf(qq|UPDATE entities
        ...
        else
        {
            ...
            log_debug(sprintf(qq|entity %s status changed: %s -> %s|, $self->id_entity, $self->status, $status), _LOG_DEBUG)
                if $LogEnabled;
...
#print "AFTER:  ", $self->[FLAP_MONITOR], "\n";
               ...
               if ($flaps_alarm_count)
               {
                   ...
                   {
#print "SERWIS FLAPUJE!!!!! " . $self->name, "\n";
                       if ($status && ! $self->[FLAP])
                       { 
                           $self->[FLAP] = time;
                       } 
...
sub errmsg
{
    ...
    {
        $_[0] = sql_fix_string($_[0]);
        if ($self->[ERRMSG] ne $_[0])
        {
            log_debug(sprintf(qq|entity %s errmsg changed: %s -> %s|, $self->id_entity, $self->errmsg, $_[0]), _LOG_DEBUG)
                if $LogEnabled;
            $self->db_save('errmsg');
            $self->[ERRMSG_OLD] = $self->[ERRMSG];
            $self->[ERRMSG] = shift;
