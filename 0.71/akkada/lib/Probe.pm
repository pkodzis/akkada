...
our $FlagsCheckTimeSlots = CFG->{Probe}->{FlagsCheckTimeSlots};
our $ForceCheckTimeSlots = CFG->{Probe}->{ForceCheckTimeSlots};
our $RRDDir = CFG->{Probe}->{RRDDir};
our $LogEnabled = CFG->{LogEnabled};
our $ThresholdMediumDefault = CFG->{Probe}->{ThresholdMediumDefault};
our $ThresholdHighDefault = CFG->{Probe}->{ThresholdHighDefault};
our $StatusDir = CFG->{Probe}->{StatusDir};
our $ProbesMaxCountPerCycle = CFG->{ProbesMaxCountPerCycle};
our $EntityTestDeltaRandomFactor = CFG->{EntityTestDeltaRandomFactor};
...
our $QueuePolicy = CFG->{Probe}->{QueuePolicy};
our $BadConfReload = CFG->{Probe}->{BadConfReload};
...
sub add_entity
{
    ...
    else
    {
        ...
        {
            open F, $file;
            ($self->entities->{ $id_entity }->{last_check}, $self->entities->{ $id_entity }->{last_delta}) = split /:/, <F>;
            close F;
        }
        except
        {
...
sub entity_load_to_cache
{
    ...
    try
    {
        log_debug("getting entity $_ from database", _LOG_INTERNAL)
            if $LogEnabled;

        $self->entity_cache->{$id_entity}->{entity} = Entity->new($self->dbh, $id_entity, 0);
    }
...
#log_debug($id_entity . ": END", _LOG_ERROR)
    ...
    ++$reload_needed->{$id_entity};
    my $entity_mass_data = get_entities_mass_data($self->dbh, [keys %$reload_needed]);

#use Data::Dumper; log_debug($id_entity . ":A2 " . Dumper($entity_cache->{$id_entity}->{entity}), _LOG_ERROR)
    ...
    { 
        ...
        for my $id_entity ( grep { $entities->{$_}->{queue} eq $queue  } keys %$entities )
        {
            ...
            elsif (flag_file_check($FlagsControlDir, sprintf(qq|%s.check_period|, $id_entity), 1))
            {
                ...
                log_debug(sprintf("entity %d check period changed to ", 
...
#use Data::Dumper; log_debug(Dumper($entity), _LOG_ERROR);
        ...
        if ($LogEnabled)
        {
            ...
            log_debug(sprintf(qq|entities in queues: %s fast, %s normal, %s slow|, scalar @qf, scalar @qn, scalar @qs), _LOG_INFO);
        }
...
{ 
    ...
    $buttons->add({ caption => "options", url => "javascript:open_location('1','"
        . $self->popup_item_url_app($view_mode)
        . "','','$class');",});
    $buttons->add({ caption => "services options", url => "javascript:open_location('2','"
        . $self->popup_item_url_app($view_mode)
        . "','','$class');",})
        if $class eq 'node';
    ...
    $buttons->add({ caption => "stop monitor", 
        url => "javascript:open_location($section,'?form_name=form_options_mandatory&monitor=0&id_entity=','current','$class');",});
...
sub stat_cell_content
{
    ...
    $table->addRow(
        $cgi->img({ onClick =>qq|bc(this, '$id')|, src => "/img/o.gif", }),
        $cgi->img
        ({ 
...
sub popup_menu_graphtime
{
    ...
    $buttons->button_refresh(0);
    $buttons->button_back(0);
    $self->popup_items_graphtime({buttons => $buttons });
...
{   
    my $self = shift;
    my $view_mode = $_[0]->{view_mode}; 
    my $settings = $_[1] ? $_[1] : 0;

    if (defined $VIEWS_ALLVIEWS{$view_mode} && ! $settings)
    {
        return qq|?form_name=form_view_select&id_view=|;
    }
    elsif ($settings)
    {
        return sprintf(qq|?form_name=form_view_mode_change&nvm=%s&id_view=|, _VM_VIEWS);
    }
    else
    {
...
{   
    ...
    $buttons->add({ caption => "settings",
        url => "javascript:open_location('0','" . $self->popup_items_view_app($_[0], 1) . "','','view');",});
}   
...
sub popup_items_find
{
    ...
    $buttons->add({ caption => "stat",
        url => "javascript:open_location('5','" . $self->popup_items_find_app($_[0]) . "','','view');",});
    $buttons->add({ caption => "log",
        url => "javascript:open_location('4','" . $self->popup_items_find_app($_[0]) . "','','view');",});
}
