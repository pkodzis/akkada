...
sub _locked
{
    ...
    {
        log_debug(qq|cache locked by slave|, _LOG_ERROR)
            if $LogEnabled;
...
sub reload_node
{
    ...
    if ($cache_save == 2)  #used by form_contacts_* for update contact groups of entity
    {
        return
...
sub init_global_db
{
    ...
    $self->{VIEWS} = $dbh->selectall_hashref("select id_view,name,status,function,UNIX_TIMESTAMP(last_change) AS last_change from views", "id_view");
}
...
sub html
{
    ...
    if (defined $VIEWS_ALLTREES{$view_mode})
    {
        ...
        for my $id_view ( sort { uc($views->{$a}->{name}) cmp uc($views->{$b}->{name}) } keys %$views)
        {
            ...
            $res .= qq|','', '', '', '$views->{$id_view}->{function}', '$views->{$id_view}->{status}','','view', 0, 0);\n|;
        }
