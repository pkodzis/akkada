...
formShowHide = function(cname, name, load, closed)
{
    ...
    var fm = tD.getElementById(name );
    var fs = tD.getElementById("_" + name );

    var fmc;
    if (closed != null) {
        var fmc = tD.getElementById(name + '_closed');
    }

    if (load == 1) {
        var val = cT[name];
        fm.style.display = val != null && val != "" ? val : "block";
        if (fmc != null) {
            fmc.style.display = fm.style.display == "block" ? "none" : "block";
        }
    } else {
        fm.style.display = fm.style.display == "none" ||  fm.style.display == "" ? "block" : "none";
        if (fmc != null) {
            fmc.style.display = fm.style.display == "block" ? "none" : "block";
        }
    }
    ...
    if (fs != null && cname != "AG") {
        if (fm.style.display == "block") {
            fs.setAttribute("class", "ww"); 
            fs.setAttribute("className", "ww"); 
        } else {
            fs.setAttribute("class", "ww0"); 
            fs.setAttribute("className", "ww0"); 
        };
    };

    if (cname == "AG") {
        var i;
        i = tD.getElementById("_"+name);
        i.src=(tD.getElementById(name).style.display=="block")?oc[0]:oc[1];
    }

    var expire=new Date();
    expire.setTime(expire.getTime()+630720000000);

    cT[name] = fm.style.display;

    cookie = '';
    for (var i in cT) {
        if (cookie != '') { cookie = cookie + '||'; }
        cookie = cookie + i + '=' + cT[i];
    }
    setCookie(cname, cookie, expire);
}
...
function nfdbc(i, n) {
  i = tD.getElementById(i);
  i.innerHTML=(i.innerHTML == 'show details')?'hide details':'show details';
  tD.getElementById(n).style.display=(tD.getElementById(n).style.display=="none")?"block":"none";
}
function nfcbc(i, n) {
  i = tD.getElementById(i);
  i.innerHTML=(i.innerHTML == 'show graph')?'hide graph':'show graph';
  tD.getElementById('nfc'+n+'_').style.display=(tD.getElementById('nfc'+n+'_').style.display=="none")?"block":"none";
  var nfci = tD.getElementById('nfc'+n+ '__');
  var x = nfci.src.split("");
  if (x[x.length-1] == '#')
      nfci.src='/nfgraph/'+n;
}
...
function socb(f, na) {
  for (i=0,n=f.elements.length;i<n;i++) {
    if (f.elements[i].name.indexOf(na) != -1) {
      f.elements[i].checked = (f.elements[i].checked==true)?false:true;
      if (na != 'update')
        tD.getElementById( f.elements[i].name.substring(0, f.elements[i].name.indexOf("_")) + "_update").checked = true;
    }
  }
}

function sotf(t, f, na) {
  var u = '';
  if (t.value == "")
    return;
  for (i=0,n=f.elements.length;i<n;i++) {
    if (f.elements[i].name.indexOf(na) != -1) {
      f.elements[i].value = t.value;
      tD.getElementById( f.elements[i].name.substring(0, f.elements[i].name.indexOf("_")) + "_update").checked = true;
    }
  }
}
...
function sour(id) {
  tD.getElementById( id + "_update").checked = true;
}
