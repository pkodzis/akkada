...
(
    ...
    _VM_TREE _VM_TREE_LIGHT _VM_VIEWS _VM_VIEWS_LIGHT _VM_TREEVIEWS _VM_FIND _VM_TREEFIND _VM_FIND_LIGHT _VM_TREEFIND_LIGHT _VM_DASHBOARD _VM_DASHBOARD_LIGHT
    %VIEWS_HARD %VIEWS_ALLTREES %VIEWS_LIGHT %VIEWS_TREE %VIEWS_FIND %VIEWS_VIEWS %VIEWS_TREEFIND %VIEWS_ALLFIND %VIEWS_ALLVIEWS %VIEWS_TREE_PURE %VIEWS_SWITCH %VIEWS_DASHBOARD %VIEWS_NONBASE
    %_ST_LIST
    _VT_STATIC _VT_FIND
    _FWD_YES _FWD_NO _FWD_UNKNOWN _IP_PRIMARY _IP_SECONDARY _A_OK _A_DOWN _A_UNUSABLE _A_STATIC_OK _A_INIT
);

use constant _VT_STATIC => 0;
use constant _VT_FIND => 1;

#Available2 constants
use constant
{
    _FWD_UNKNOWN => 0,
    _FWD_YES => 1,
...
use constant _VM_DASHBOARD_LIGHT => 15;

our %VIEWS_NONBASE =
(
    5  => 1, # _VM_DASHBOARD      => _VM_DASHBOARD_LIGHT
    15 => 1,  # _VM_DASHBOARD_LIGHT=> _VM_DASHBOARD
);
...
(
    ...
    5  => 15, # _VM_DASHBOARD      => _VM_DASHBOARD_LIGHT
    15 => 5,  # _VM_DASHBOARD_LIGHT=> _VM_DASHBOARD
