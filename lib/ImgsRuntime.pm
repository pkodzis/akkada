    ...
    for $file ( sort { uc $a cmp uc $b } @files )
    {
        ...
        if ($form && $field)
        {
            $table->setCell($row, $col, 
                qq|<a href="javascript:select_img('$name');"><img src="/img/$file" class="b10" alt="$name" width=16 height=16/></a>|);
        }
        else
        {
            $table->setCell($row, $col, qq|<img src="/img/$file" class="b10" alt="$name" width=16 height=16/>|);
        }
