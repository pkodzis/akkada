...
our $RRDDir;
our $LogEnabled;
our $MaxSNMPSplitRequest;
our $DEFDir;
our $LowLevelDebug;
our $ComputeDebug;
our $ThresholdDefaults;
...
sub got_sig_hup
{
    reload_cfg();
    init_globals();
    log_debug(qq|configuration reloaded|, _LOG_ERROR)
        if $LogEnabled;
}
...
{
    Probe::init_globals;
    $DataDir = CFG->{Probe}->{DataDir};
    $RRDDir = CFG->{Probe}->{RRDDir};
    $LogEnabled = CFG->{LogEnabled};
    $MaxSNMPSplitRequest = CFG->{MaxSNMPSplitRequest};
    $DEFDir = CFG->{Probes}->{snmp_generic}->{DEFDir};
    $LowLevelDebug = CFG->{Probes}->{snmp_generic}->{LowLevelDebug};
    $ComputeDebug = CFG->{Probes}->{snmp_generic}->{ComputeDebug};
    $ThresholdDefaults = CFG->{Probes}->{snmp_generic}->{ThresholdDefaults};
}
...
sub compute
{
    ...
    for my $compute (@$computes)
    {
        ...
        if ($ComputeDebug)
        {
            ...
            log_debug("CURR COMPUTE:" . $compute->[1] . ":", _LOG_ERROR);
            warn("CURR COMPUTER:" . $compute->[1] . ":");
            warn("CURR COMPUTER:" . $compute->[2] . ":");
            warn("CURR COMPUTER:" . $mask . ":");
            warn("CURR COMPUTER:" . ($compute->[2] & $mask ? "OK" : "BAD"));
        }
        ...
        {
            $tmp = $self->compute_percdefl($compute, $data);
            $res->{value} = $tmp
                unless $compute->[0];
        }
        ...
        elsif ($compute->[1] =~ /^PERCENTLOW::/ && $compute->[2] & $mask)
        {
            $tmp = $self->compute_percentlow($compute, $data);
            $res->{value} = $tmp
                unless $compute->[0];
        }
        elsif ($compute->[1] =~ /^TICK2DURATION::/ && $compute->[2] & $mask)
        {
            $tmp = $self->compute_tick2duration($compute, $data);
            $res->{value} = $tmp
                unless $compute->[0];
        }
        ...
        elsif ($compute->[1] =~ /^HEXSTR2IP::/ && $compute->[2] & $mask)
        {
            $compute->[1] =~ /^HEXSTR2IP::%%(.*)%%$/;
            $tmp = hex_str_to_ip( $data->{ $1 } );
            $res->{value} = $tmp
                unless $compute->[0];
        }
        elsif ($compute->[1] =~ /^%%.*%%$/ && $compute->[2] & $mask)
        {
            ...
            $tmp = ($self->compute($data, [[$compute->[0], $tmp]], $mask))[0]; #wykorzystywane np. w power_power_input frequency
            $res->{value} = $tmp
                unless $compute->[0];
        }
...
#warn "XXXX: $vlow $vhigh $value: vl: $vlow vh: $vhigh y $y flag: $flag z $z x $x";
    ...
    elsif (! $simple)
    {
        if ($deflection)
        {
            $value = sprintf(qq|<font class="%s">%s %s (deflection -%.2f%)</font>|, percent_bar_style_select($x, 1),$value, $units, $x);
        }
        else
        {
            $value = sprintf(qq|<font class="%s">%s %s</font>|, percent_bar_style_select($x, 1),$value, $units);
        }
    }
    else
    {
...
sub compute_percent
{
    ...
    $value = $value ne 'n/a'
        ? sprintf(qq|<font class="%s">%s %%</font>|, percent_bar_style_select($value),$value)
        : 'n/a';
...
sub compute_percentlow
{
    ...
    if ($ComputeDebug)
    {
        warn "PERCENTLOW START: $value";
        log_debug("PERCENTLOW START: $value", _LOG_ERROR);
    }
    ...
    $value = $data->{ $1 } ne 'n/a'
        ? sprintf(qq|<font class="%s">%s %%</font>|, percent_bar_style_select($data->{$1}, 2),$data->{$1})
        : 'n/a';
    ...
    if ($ComputeDebug)
    {
        warn "PERCENTLOW END: $data_key#: $value";
        log_debug("PERCENTLOW END: $data_key#: $value", _LOG_ERROR);
    }

    return $value;
}

sub compute_tick2duration
{
    my $self = shift;
    my $value = shift;
    my $data = shift;
    ...
    if ($ComputeDebug)
    {
        warn "TICK2DURATION START: $value";
        log_debug("TICK2DURATION START: $value", _LOG_ERROR);
    }
    ...
    if ($ComputeDebug)
    {
        warn "TICK2DURATIONEND: $data_key#: $value";
        log_debug("TICK2DURATION END: $data_key#: $value", _LOG_ERROR);
    }
...
sub compute_format
{
    ...
    elsif ($value =~ /^FORMAT::HEX2STRING/)
    {
        $value =~ /^FORMAT::HEX2STRING::%%(.*)%%/;
        $value = pack("H*", $data->{ $1 });
    }
    else
    {
...
sub compute_rpn
{
    ...
    $value =~ s/KHZ/kHz/g;
    $value =~ s/MHZ/MHz/g;
    $value =~ s/FONTCLASS/FONT CLASS/g;
...
{  
    ...
    {
        next
            unless defined $trackmap->{$track_name};
        next
        ...
        else
        {
            ...
            if (defined $text_test->{ignore_unless})
            {
                if (defined $data->{$text_test->{ignore_unless}->{name}} )
                {
                    next
                        unless $data->{$text_test->{ignore_unless}->{name}} =~ /$text_test->{ignore_unless}->{value}/;
...
sub discover_def
{
    ...
    my $discover_index_override = defined $definition->{DISCOVER_INDEX_OVERRIDE} ? $definition->{DISCOVER_INDEX_OVERRIDE} : undef;
    ...
    for my $oid (@{ $definition->{DISCOVER} })
    {
        ...
        for $discover_index (keys %$result)
        {
            $st = $result->{$discover_index};
            $discover_index =~ s/^$oid\.//g;
            if (defined $discover_index_override)
            {
                @tmpdio = split /\./, $discover_index;
                $discover_index = $tmpdio[ $discover_index_override ];
            }
            $st =~ s/\'//g;
            $st =~ s/\000//g;
            $st =~ s/\s+$//;        
            push @{ $tmp->{ $discover_index } }, $st;
        }
    }
    ...
    for $name (keys %$old)
    {
        ...
        $old->{$name}->{entity}->params('index', $new->{$name}->{discover_index})
             if defined $discover_index 
                 && $discover_index ne '' 
                 && defined $new->{$name}->{discover_index}
                 && defined $old->{$name}->{index}
                 && $new->{$name}->{discover_index} ne $old->{$name}->{index};
...
{       
    ...
    for my $track_name (keys %$data)
    {
        ...
        print F sprintf(qq|%s\|%s\n|, $track_name, defined $data_value ? $data_value : '');
    }   
