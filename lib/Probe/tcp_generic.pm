...
our $DiscoverPreventFirewallFakes = CFG->{Probes}->{tcp_generic}->{DiscoverPreventFirewallFakes} || 16;
...
sub discover 
{
    ...
    elsif (@result && $#result > $DiscoverPreventFirewallFakes)
    {
...
sub desc_brief
{
    ...
    {   
        push @$result, sprintf(qq|duration: %s ms|, $data->{duration}*1000);
    }
    else
    {   
...
sub desc_full_rows
{
    ...
    $table->addRow('duration', sprintf(qq|%s ms|, $data->{duration}*1000));
