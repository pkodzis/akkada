...
#use Data::Dumper; log_debug(Dumper($data),_LOG_ERROR);
        ...
        if (defined $expected && ref($expected) eq 'ARRAY')
        {
            for (@$expected)
            {
                if ($output !~ /$_/)
                {
                    $self->errmsg(defined $data->{text}->{errmsg} && $data->{text}->{errmsg}
                        ? $data->{text}->{errmsg}
                        : sprintf(qq|expected string "%s" not found|,$_));
                    $self->status(_ST_DOWN);
                }
            }
        }
        if (defined $bad && ref($bad) eq 'ARRAY')
        {
            ...
            {   
                if ($output =~ /$_/)
                {
                    $self->errmsg(defined $data->{text}->{errmsg} && $data->{text}->{errmsg}
                        ? $data->{text}->{errmsg}
                        : sprintf(qq|bad string "%s" found|,$_));
                    $self->status(_ST_DOWN);
                }
            }   
        }
...
sub desc_full_rows
{
    ...
    if ($data_type eq _DT_TEXT)
    {
        $data->{output} =~ s/\</\&lt\;/g;
        $data->{output} =~ s/\>/\&gt\;/g;
        $data->{expected} =~ s/\</\&lt\;/g;
        $data->{expected} =~ s/\>/\&gt\;/g;
        $data->{bad} =~ s/\</\&lt\;/g;
        $data->{bad} =~ s/\>/\&gt\;/g;
        $data->{errmsg} =~ s/\</\&lt\;/g;
        $data->{errmsg} =~ s/\>/\&gt\;/g;
        $data->{extOutput} =~ s/\</\&lt\;/g;
        $data->{extOutput} =~ s/\>/\&gt\;/g;
...
sub prepare_ds_stat
{
    ...
    for (@$title)
    {
        ...
        push @$args, "LINE1:ds$i#$colors->[$i-1]:$_";
    }
