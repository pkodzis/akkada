...
#print "error oid: ",  $oids->[2]->[$session->error_index - 1], "\n\n";
        ...
        else
        {
            ...
            my $ifEntry = $self->ifEntry;

            if (! $self->status)
            {
                if ($nic_ambiguous_ifDescr)
                {
                    $entity->name( sprintf(qq|%s.%s|, defined $ifEntry ? $ifEntry->{ifDescr} : '', $index));
                }
                else
                {
                ...
                my $ifAdminStatus = $ifEntry->{ifAdminStatus};
                my $ifOperStatus = $ifEntry->{ifOperStatus};
                ...
                elsif ( defined $ifOperStatus && $ifOperStatus eq 'dormant'
                    && $entity->params('nic_ifOperStatus_interpret_dormant_down'))
                {
                    if (! $nic_ifOperStatus_invert)
                    {
                        $self->errmsg(sprintf(qq|interface ifOperStatus: %s|, $ifOperStatus));
    ...
    if ($self->status < _ST_DOWN)
    {
        ...
        $entity->params('ip_addresses', $self->ipAddrEntry);
...
sub errors_and_utilization_status
{
    ...
    if  ($speed && $ch->{ifInOctets}->[1] ne 'U' && $ch->{ifOutOctets}->[1] ne 'U')
    {
        $ch->{top10} = ( ($ch->{ifInOctets}->[1] + $ch->{ifOutOctets}->[1])*800 ) / $speed;
    }
    else
    {
        $ch->{top10} = undef;
...
sub save_data
{
    ...
    top_save($id_entity, 'nic', $ch->{top10});
}
...
sub discover
{
    ...
    if (! $error)
    {
        ...
        if (! keys %$new)
        {
            log_debug(sprintf("no network interfaces available on the entity %s", $entity->id_entity) , _LOG_INFO)
                if $LogEnabled;
            return;
        }
...
{   
    ...
    {   
        ...
        my $icmp; 
        if (@$tmp == 1 && $entity->status < _ST_UNKNOWN && ! $entity->params('nic_ip_icmp_check_disable'))
        {   
            $icmp = icmp_get_stats($tmp->[0]->[0]);
            if ($icmp)
            {   
                push @$result, $icmp;
            }
        ...
        {   
            ...
            {   
                next
                    unless $_->[2];
                if ($entity->status < _ST_UNKNOWN && ! $entity->params('nic_ip_icmp_check_disable')) 
                {
                    $icmp = icmp_get_stats($_->[0]);
                    if ($icmp)
                    {   
                        push @$result, $icmp;
                    }
    ...
    if ($data->{ipAddrEntry})
    {
        ...
        else
        {
            for (@$tmp)
            {
                ...
                $table->addRow( "ip address " . ($_->[2] ? ' primary' : ' secondary') . ":", $s );
                if ($entity->status < _ST_UNKNOWN && ! $entity->params('nic_ip_icmp_check_disable'))
                {
                    $icmp = icmp_get_stats($_->[0]);
                    if ($icmp)
