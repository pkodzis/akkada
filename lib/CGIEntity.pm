...
sub entity_info
{
    ...
    $table->addRow( "", "name", "", "", "last change", "age of data","");
    $table->setCellAttr(1, 2, 'class="g4"');
    $table->setCellAttr(1, 5, 'class="g4"');
    $table->setCellAttr(1, 6, 'class="g4"');

    my ($status, $last_check, $check_period) = $self->entity_row_main($table);

    $self->entity_row_main_style($table, 2, $status, $last_check, $check_period);
...
sub entity_parameters_optional
{
    ...
    {   
        $table->addRow("&nbsp;$_&nbsp;",
            $_ =~ /password/ || $_ =~ /community/ || $_ =~ /ipsec_enable/ ? "*****" : $params->{$_},
...
sub alarms_conditions_meet
{
    ...
    for my $c (keys %$cond)
    {
        ...
        if ($c->{field} eq 'parent')
        {
            ++$meet
                if $self->cond_check($c->{cond}, $c->{value}, $parent->name);
        }
        elsif ($c->{field} eq 'name')
        {
            ++$meet
                if $self->cond_check($c->{cond}, $c->{value}, $node->name);
        }
        elsif ($c->{field} eq 'status')
        {
            ++$meet
                if $self->cond_check($c->{cond}, $c->{value}, status_name($node->status));
        }
        elsif ($c->{field} eq 'errmsg')
        {
            ++$meet
                if $self->cond_check($c->{cond}, $c->{value}, $node->errmsg);
        }
    }
...
sub cond_check
{
    ...
    my $cond = shift;
    my $value = shift;
    my $value_to_check = shift;
#warn "$cond: $value: $value_to_check";
    if ($cond eq 'equal')
    {
        return 1
            if $value_to_check =~ /^$value$/gi;
    }
    elsif ($cond eq 'not_equal')
    {
        return 1
            if $value_to_check !~ /^$value$/gi;
    }
    elsif ($cond eq 'contain')
    {
        return 1
            if $value_to_check =~ /$value/gi;
    }
    elsif ($cond eq 'not_contain')
    {
        return 1
            if $value_to_check !~ /$value/gi;
    }
...
#use Data::Dumper; warn Dumper $view_entities;
    ...
    my $conditions = session_get_param($self->session, '_ALARMS_CONDITIONS');
    $conditions = undef
        unless defined $conditions && ref($conditions) eq 'HASH';
    ...
    for my $id (keys %$nodes)
    {
        ...
        next
            if defined $conditions && ! $self->alarms_conditions_meet($nodes->{$id}, $items->{$rel->{$id}}, $conditions);
    ...
    my ($unreachable_corr, $unreachable_corr_rels);
    ($unreachable_corr, $unreachable_corr_rels) = $self->unreachable_corr_load
        if $correlation;

    my $entities_mass_data = {};
    $entities_mass_data = get_entities_mass_data($dbh, [keys %allids])
        if keys %allids;
    ...
    for my $id (@job)
    {
        try
        {
       $entities->{$id} = Entity->new( $dbh, $id, 1, $entities_mass_data->{$id});
       $entities->{ $rel->{$id} } = Entity->new( $dbh, $rel->{$id}, 1, $entities_mass_data->{ $rel->{$id}} )
...
#use Data::Dumper; warn Dumper($correlate);
        ...
        } @job)
    {
        ...
        next
            if $correlation 
            && (defined $correlate->{$id}
            || defined $unreachable_corr_rels->{$id});
        ...
        ($status, $last_check_timestamp, $last_check, $check_period, $err_approved_by)
            = $self->alarm_row($table, $nodes->{$id}, $items->{$rel->{$id}}, $url_params, $id, $entities);
        $self->alarm_row_style($table, $table->getTableRows, $status, $color, $last_check_timestamp, $last_check, $check_period, $err_approved_by);
        $color = ! $color;
        $parent_count++;
        $count++;
    }
    ...
    return scalar $table
        if ! $correlation && $table->getTableRows > 2; 

    if (keys %$unreachable_corr)
    {
        $color = 0;

        my $cgi = $self->cgi;

        my $nftable = $self->alarms_nftable_begin();
        ...
        for my $corrid (keys %$unreachable_corr)
        {
            $corr = $unreachable_corr->{$corrid}->{correlation};

            ($brief, $msg, $rc, $hosts) = $self->unreachable_corr_data_get($corr, $corrid);

            $nfapprove = 'approved';
            for (keys %$rc, @$hosts)
            {
                if (defined $nodes->{$_} && ! $nodes->{$_}->err_approved_by)
                {
                    $nfapprove = '';
                    last;
                }
            ...
            next
                if $url_params->{hide_approved} && $nfapprove;

            if (! $nfapprove)
            {
                $nfapprove = $cgi->a({ 
                     href => sprintf(qq|%s?%s|, url_get({}, $self->url_params), 'form_name=form_alarm_approval&atype=4&id_entity=0&entities=' . join(",", keys %$rc, @$hosts)), class=> 'info' }, 
                     $cgi->img({alt => 'approve correlated alarm', src => '/img/aac.gif'}));
            }

            $t = HTML::Table->new(-spacing=>0, -padding=>0);
            $t->setAttr('class="e"');

            $t->addRow(
                #$cgi->img({ onClick =>qq|bc(this, 'nfc$nfcid')|, src => "/img/c.gif", }) . "&nbsp;" .
                #$cgi->img({ onClick =>qq|nfcbc(this, '$corrid')|, src => "/img/c.gif", }) . "&nbsp;" .
                $brief);

            #$t->addRow(qq|<a href="javascript:nfdbc('nfc${nfcid}__', 'nfc$nfcid');"><div id="nfc${nfcid}__">show details</div></a>|,
            #    qq|<a href="javascript:nfcbc('${corrid}__', '$corrid');"><div id="${corrid}__">show graph</div></a>|);

            $t->addRow(sprintf(qq|<div id="nfc%s" style="display: none;">%s</div>|, $nfcid, $msg));

            #$t->setCellColSpan(1, 1, 2);
            #$t->setCellColSpan(3, 1, 2);

            $nftable->addRow(
                $self->unreachable_corr_build_rc([keys %$rc]),
                status_name(_ST_UNREACHABLE),
                duration($corr->{timestamp}),
                qq|<a class="g22" href="javascript:nfdbc('nfc${nfcid}__', 'nfc$nfcid');"><div id="nfc${nfcid}__">show details</div></a>| .
                qq|<a class="g22" href="javascript:nfcbc('${corrid}__', '$corrid');"><div id="${corrid}__">show graph</div></a>|,
                scalar $t,
                $nfapprove,
            );
            $self->nftable_row_style($nftable, $color, $nfapprove);
            $nftable->addRow(qq|<div id="nfc${corrid}_" style="display: none;"><img id="nfc${corrid}__" src="#"/></div>|);
            $nftable->setCellColSpan($nftable->getTableRows, 1, 5);
            $nftable->setCellAttr($nftable->getTableRows, 1, 'class="g3"');
            $nftable->setRowAttr($nftable->getTableRows, sprintf(qq|class="tr_%d"|, $color));
            $nftable->setCellAttr($nftable->getTableRows, 6, $nfapprove eq 'approved' ? 'class="g1"' : 'class="m"');

            ++$nfcid;
            $color = ! $color;
        }

        if ($nftable->getTableRows > 2)
        {
        $tmain->addRow
        ( 
            $cgi->img({ onClick =>qq|bc(this, 'network_faults')|, src => "/img/o.gif", }), 
            sprintf(qq|<div id="network_faults">%s</div>|, scalar $nftable),
        );
        ...
        } @{ $correlate->{links}->{$cor} })
    {
        ...
        $self->alarm_row_style($table, $table->getTableRows, $status, $color, $last_check_timestamp, $last_check, $check_period, $err_approved_by, 1, $color);
        $count++;
    }
...
#DUPA
    for (@$rc)
    {
        ...
        $err = $node->flap ? sprintf(qq|%d flaps since %s; <font color="#606060">last error: "%s"</font>|,
            $node->flap_count, duration($node->flap), $node->errmsg) : $node->errmsg;

        $s =  $self->a_popup({ id => $_, probe => $ProbesMapRev->{$node->id_probe_type}, name => $node->name, section => 'general' });

        if ($node->id_probe_type > 1)
        {
            $node = $tree->get_node($rel->{$_});
            $s = sprintf(qq|%s :: %s|,
                $self->a_popup({ id => $rel->{$_}, probe => 'node', name => $node->name, section => 'general' }),
                $s);
        }

        push @result, qq|$s<br><font class="g9">$err</font>|;
    }
   
    return join("<br>", sort { uc $a cmp uc $b } @result); 
}

sub unreachable_corr_data_get
{
#DUPA
    my ($self, $corr, $corrid) = @_;
    my ($msg, $brief);

    my $tree = $self->tree;

    my ($rc, $hosts, $nets) = unreachable_corr_extract($corr);

    $brief = '<font class="g9">because of the root cause ' . scalar @$hosts . " hosts and " . scalar @$nets . " networks are unavailable.</font><br>";
    $msg = '<font class="s"><b>affected hosts: </b></font>' 
        . join(", ", 
            map { $self->a_popup({ id => $_, probe => 'node', name => $tree->get_node($_)->name, section => 'general' }) }
            sort { uc $a cmp uc $b }  @$hosts) 
        . "<br>"
        if @$hosts;
    $msg .= '<font class="s"><b>affected networks: </b>' . join(", ", sort @$nets) . "</font>" #<br>"
        if @$nets;

#$t->addRow($cgi->img({ onClick =>qq|bc(this, 'nfc$nfcid')|, src => "/img/c.gif", }) . "&nbsp;" . $brief);
    #$msg .= qq|<a href="/nfgraph/$corrid" target="$corrid"><font class="g96">visualize on the graph...</font></a>|;
    #$msg .= qq|<a href="javascript:nfcbc('$corrid');"><font class="g96">visualize on the graph...</font></a>|;

    return ($brief, $msg, $rc, $hosts);
}
...
sub alarms_nftable_begin
{
    my $self = shift;
    my $cgi = $self->cgi;
#DUPA
    my $table = table_begin('network faults', 5);
...
sub nftable_row_style
{
    ...
    $table->setCellAttr($row, 1, 'class="g3"');
    $table->setCellAttr($row, 2, qq|class="ts| . _ST_UNREACHABLE . qq|"|);
    $table->setCellAttr($row, 3, 'class="g22"');
    $table->setCellAttr($row, 4, 'class="g22"');
    $table->setCellAttr($row, 5, 'class="g33"');
    $table->setCellAttr($row, 6, $nfapprove eq 'approved' ? 'class="g1"' : 'class="m"');
...
{   
    ...
    my $table = table_begin($correlation ? 'correlated alarms' : 'alarms', 12);
...
{   
    ...
    $last_check_timestamp ne 'n/a' && $check_period && time - $last_check > $check_period * $OldLastCheckAlarm
...
sub a_popup
{
    ...
    my $top_level = defined $h->{ top_level } ? $h->{ top_level } : 0;
    my $caption = defined $h->{ caption} ? $h->{ caption} : 0;
    ...
    if (! $caption && (defined $VIEWS_ALLVIEWS{$view_mode} || defined $VIEWS_ALLFIND{$view_mode} || $url_params->{section} eq 'dashboard'))
    {
...
=cut
    ...
    push @result, $self->contacts_img($entity->id_entity) 
        . $self->comments_img($entity->id_entity) 
        . $self->actions_img($entity->id_entity);
    ...
    return ($status, $result[$#result - 1], $entity->data->{last_check}, $entity->check_period, $err_approved_by);
}
...
sub services_options
{
    ...
    $cont->{form_title} = 'service options';
    ...
    [
        ...
        '',
        '',
        $cgi->checkbox({name => "_ignore", label => "", onClick => "javascript:socb(this.form, 'delete');"}),
        '',
        $cgi->textfield({ name => "_ignore", value => "", class => "textfield", size => 1, onBlur => "javascript:sotf(this, this.form, 'status_weight');"}),
        $cgi->textfield({ name => "_ignore", value => "", class => "textfield", size => 1, onBlur => "javascript:sotf(this, this.form, 'check_period');"}),
        $cgi->textfield({ name => "_ignore", value => "", class => "textfield", size => 1, onBlur => "javascript:sotf(this, this.form, 'monitor');"}),
        '',
        $cgi->checkbox({name => "_ignore", label => "", onClick => "javascript:socb(this.form, 'update');"}),
    ];
        ...
        keys %$children)
    {
        ...
        for (@$services_options)
        {
            ...
            push @tmp, $cgi->textfield({ name => sprintf(qq|%d_%s|, $id_entity, $s) , 
                ...
                size => $_->[1],
                onChange => "javascript:sour($id_entity);",
                }),
        }
        ...
        push @{ $cont->{rows} },
            [
                ...
                status_name($s),
                $cgi->checkbox({name => "${id_entity}_delete", label => "", onClick => "javascript:sour($id_entity);"}),
                $items->{$id_entity}->errmsg,
                @tmp,
                $cgi->checkbox({name => "${id_entity}_update", label => ""}),
            ];

        push @{ $cont->{class} }, [ scalar @{ $cont->{rows} } + 4, 1, 't2'];
        push @{ $cont->{class} }, [ scalar @{ $cont->{rows} } + 4, 2, 'f'];
        push @{ $cont->{class} }, [ scalar @{ $cont->{rows} } + 4, 3, sprintf(qq|ts%s|, $s)];
        push @{ $cont->{class} }, [ scalar @{ $cont->{rows} } + 4, 5, 'g9'];
        push @{ $cont->{class} }, [ scalar @{ $cont->{rows} } + 4, 6 + @tmp, 'g9'];
    }
...
sub entity_get_children
{
    ...
    my $entities_mass_data = get_entities_mass_data($dbh, \@ids);
    ...
    for (@ids) 
    {
        ...
        {
            $children->{nodes}->{$_} = Entity->new($dbh, $_, undef, $entities_mass_data->{$_});
        }
        else
        {
            $children->{leafs}->{$_} = Entity->new($dbh, $_, undef, $entities_mass_data->{$_});
        }
...
#use Data::Dumper; warn Dumper($view_entities);
    ...
    for (@ids)
    {
        try
        {
            push @$entities, Entity->new($dbh, $_, undef, $entities_mass_data->{$_});
        }
        catch  EEntityDoesNotExists with
        {
        }
...
sub entity_very_short
{
    ...
    {
        if (defined $VIEWS_ALLVIEWS{ $self->desktop->view_mode} && $self->desktop->views->id_view_type == _VT_STATIC)
        {
...
#use Data::Dumper; warn Dumper $children;
    ...
    for my $id_entity (
        ...
        keys %$children)
    {   
        ($status, $last_check, $check_period) = $self->entity_row($table, $children->{$id_entity});
        $self->entity_row_style($table, $table->getTableRows, $status, $children->{$id_entity}, $last_check, $check_period);
        $table->setRowClass($table->getTableRows, sprintf(qq|tr_%d|, $color));
        $color = ! $color;
    }
    ...
    for my $entity (@$children)
    {
        ($status, $last_check, $check_period) = $self->entity_row($table, $entity) ;
        $self->entity_row_style($table, $table->getTableRows, $status, $entity, $last_check, $check_period);
        $table->setRowClass($table->getTableRows, sprintf(qq|tr_%d|, $color));
        $color = ! $color;
    }
    ...
    for my $entity (
        sort {($a->id_probe_type <=> $b->id_probe_type || uc $a->name cmp uc $b->name )}
        @$children)
    {   
        ($status, $last_check, $check_period) = $self->entity_row($table, $entity) ;
        $self->entity_row_style($table, $table->getTableRows, $status, $entity, $last_check, $check_period);
...
sub entity_row_params
{
    ...
    [
        ...
        #$entity->id_probe_type < 2 ? $cgi->hidden({name => "check_period_children", value => "on", }) : '',
    ...
    [
        ...
        #$entity->id_probe_type < 2 ? $cgi->hidden({name => "monitor_children", value => "on", }) : '',
    ...
    for (sort { uc($a) cmp uc($b) } keys %$params_all)
    {
        if (defined $params->{$_})
        {
            push @{ $cont->{rows} }, ["&nbsp;$_&nbsp;",
                $params_ro->{$_}->{ro} ? $params->{$_} : 
                    ($_ =~ /password/ || $_ =~ /community/ || $_ =~ /ipsec_enable/)
        ...
        {
            push @{ $cont->{rows} }, ["&nbsp;$_&nbsp;", 
                $_ =~ /password/ || $_ =~ /community/ || $_ =~ /ipsec_enable/ ? "*****" : $params_all->{$_},
                '', 'inherited',
                ];
        }
    }

    push @{ $cont->{rows} }, [ "&nbsp;flap_monitor&nbsp;", $self->entity->flap_monitor, '', '<nobr>read only</nobr>'];
...
{ 
    ...
    $last_check ne 'n/a' && $check_period && time - $self->entity->data->{last_check} > $check_period * $OldLastCheckAlarm
...
=cut
    ...
    push @result, $self->contacts_img($entity->id_entity)
        . $self->actions_img($entity->id_entity, 1);

    $table->addRow(@result);

    return ($status, $result[$#result - 1], $entity->check_period);
}
...
=cut
    ...
    push @result, $self->contacts_img($id_entity, 1) 
        . $self->comments_img($id_entity, 1)
        . $self->actions_img($id_entity, 1);
...
{ 
    ...
    if (ref($entity->data))
    {
        $last_status ne 'n/a' && $check_period && time - $entity->data->{last_check} > $check_period * $OldLastCheckAlarm
...
sub stat
{
    ...
    if ($probe->menu_stat($entity) eq '2' && $desktop->matrix($url_params->{section}, $items->{$entity->id_entity}))
    {
        ...
        my $table;
        my $dbh = $self->dbh;
        my ($status, $last_check, $check_period, $rowc, $probe_name);
        ...
        if ($show_node_info)
        {
            ...
            if ($show_collected_info)
            {
                ($status, $last_check, $check_period) = $self->entity_row($table, $entity, 0);
                $self->entity_row_style($table, $table->getTableRows, $status, $entity, $last_check, $check_period, 0);
...
#services
        ...
        for my $id (@ids)
        {
            ...
            try
            {
                $ent = Entity->new( $dbh, $id, undef, $entities_mass_data->{$id});
            }
            catch  EEntityDoesNotExists with
            {
            }
            ...
            if (! $table_services_started)
            {
                ++$table_services_started;
            ...
            if ($show_collected_info)
            {
                ...
                ($status, $last_check, $check_period) = $self->entity_row($table, $ent, 0);
                $self->entity_row_style($table, $table->getTableRows, $status, $ent, $last_check, $check_period, 0);
                $table->setRowClass($table->getTableRows, qq|tr_0|);
            }
        ...
        $content->addRow( scalar $table )
            if defined $table && $table->getTableRows;
    }
}
...
sub stat_view
{
    ...
    my ($status, $last_check, $check_period, $rowc, $probe);
    ...
    for my $ent (@$entities)
    {
        ...
        if ($show_collected_info)
        {
            ($status, $last_check, $check_period) = $self->entity_row($table, $ent, 1);
            $self->entity_row_style($table, $table->getTableRows, $status, $ent, $last_check, $check_period, 1);
            $table->setRowClass($table->getTableRows, qq|tr_0|);
        }
...
sub form_alarms_filter
{
    ...
    $cont->{form_name} = 'form_alarms_filter';
    $cont->{form_title} = '',
    $cont->{no_border} = 1,
    $cont->{id_entity} = defined $self->entity ? $self->entity->id_entity : 0;
    ...
    push @{ $cont->{buttons} }, { caption => "update conditions", url => "javascript:document.forms['form_alarms_filter'].submit()" };
    ...
    if (defined $conditions && ref($conditions) eq 'HASH')
    {
        for (sort keys %$conditions)
        {
            ...
            push @{$cont->{cellColSpans}}, [scalar @{ $cont->{rows} }, 1, 2];
        }
        push @{ $cont->{rows} }, ['', '', ''];
    ...
    push @{ $cont->{rows} },
    [ 
        'filter field:<br>' . $cgi->popup_menu(-name=>'field', 
            -values=>[ sort { uc $items->{$a} cmp uc $items->{$b} } keys %$items], 
            -labels=> $items, -default => '', -class => 'textfield'),
        '<nobr>by condition:</nobr><br>' . $cgi->popup_menu(-name=>'cond', 
            -values=>[ sort { uc $cond->{$a} cmp uc $cond->{$b} } keys %$cond], 
            -labels=> $cond, -default => '', -class => 'textfield'),
        'value:<br>' .
        $cgi->textfield({-name =>'value', -value => '', class => "textfield"}),
    ];

    return form_create($cont) . <<EOF;
<SCRIPT LANGUAGE="javascript">
document.forms['form_alarms_filter'].field.value='';
document.forms['form_alarms_filter'].cond.value='';
document.forms['form_alarms_filter'].value.value='';
</SCRIPT>
EOF
...
sub form_history_filter
{
    ...
    return form_create($cont) . <<EOF;
<SCRIPT LANGUAGE="javascript">
document.forms['form_history_filter'].field.value='';
document.forms['form_history_filter'].cond.value='';
document.forms['form_history_filter'].value_field.value='';
document.forms['form_history_filter'].value.value='';
</SCRIPT>
EOF
...
sub entity_add
{
    ...
    {
        $content->addRow($self->form_group_add);
    }
    if ($what eq '2'
        && $self->desktop->matrix('form_node_add', $node))
    {
        $content->addRow($self->form_node_add);
    }
    if ($what eq '3'
        ...
        && $self->desktop->matrix('form_probe_select', $node))
    {
        ...
        $table->addRow($self->form_service_add);
        #$table->addRow($self->form_probe_select);
...
sub form_group_add
{
    ...
    if (defined $name && $name->id_probe_type)
    {
        return 'only groups and root can contain group entities';
    }
...
sub form_node_add
{
    ...
    if (! defined $name || $name->id_probe_type)
    {
        return 'only groups can contain node entities';
    }
    ...
    push @{ $cont->{rows} }, ['ip address or DNS name', $cgi->textfield({ name => 'ip', value => '', class => "textfield",})];
    push @{ $cont->{rows} }, ['name', $cgi->textfield({ name => 'name', value => '', class => "textfield",})];
    ...
    push @{ $cont->{rows} }, 
        ['disable ICMP availability check', 
        $cgi->textfield({ name => 'availability_check_disable', value => '', class => "textfield",}), 'set 1 if it is not possible to ping host from AKK@DA server'];
    ...
    push @{ $cont->{rows} }, 
        ['',
          $cgi->checkbox({name => "check_snmp", label => "check SNMP configuration before adding", -checked=>'checked' })
        ];
...
sub form_service_add
{
    ...
    if (! defined $name || $name->id_probe_type != 1)
    {
        return 'only nodes can contain service entities';
    }
    ...
    $result = $self->form_probe_select;

    if (! defined $url_params->{id_probe_type} || ! $url_params->{id_probe_type})
    {
        $url_params->{id_probe_type} = $url_params->{form}->{id_probe_type}
            if defined $url_params->{form} && defined $url_params->{form}->{id_probe_type} && $url_params->{form}->{id_probe_type};
    }
    return $result
        unless defined $url_params->{id_probe_type} && $url_params->{id_probe_type};
    ...
    #$table->addRow( form_create($cont) );
    return $result . "<br>" . form_create($cont);
}
...
sub actions_img
{
    ...
    my $actions = $items->{ $id_entity }->actions;

    return ''
        unless keys %$actions;

    my @myown = grep { $actions->{$_} == 1 } keys %$actions;

    return ''
        if $no_inherited && ! @myown;
