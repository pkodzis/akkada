...
sub process
{
    ...
    eval
    {
        ...
        $db->dbh->do(sprintf(qq|INSERT INTO rights(id_entity,id_group,vie,vio,com,cmo,ack,mdy,cre,del,disabled)
    ...
    return [1, qq|right for selected entity and selected group already exists|]
        if $@ =~ /duplicate/i;
    return [1, $@]
        if $@;
