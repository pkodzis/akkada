...
sub load_mysql_status
{
    ...
    return 
    {   
        Uptime => format_string_state('n/a', 1),
        Threads => format_string_state('n/a', 1),
        Questions => format_string_state('n/a', 1),
        'Slow queries' => format_string_state('n/a', 1),
        Opens => format_string_state('n/a', 1),
        'Flush tables' => format_string_state('n/a', 1),
        'Open tables' => format_string_state('n/a', 1),
        'Queries per second avg' => format_string_state('n/a', 1),
        'Last check' => 'n/a',
    }
    ...
    $result->{'Last check'} = (stat($MySQLStatusFile))[9];
...
sub form_status_mysql
{
    ...
    my @f = (localtime($mysql->{'Last check'}))[0..5];
    $f[4] += 1;

    ++$err
        if ($mysql->{'Last check'} eq 'n/a' || time - $mysql->{'Last check'} > $AkkadaMgmtPeriod*3);
    $mysql->{'Last check'} = format_string_state(
        $mysql->{'Last check'} eq 'n/a' 
            ? 'n/a' 
            : sprintf("%02d-%02d-%d, %02d:%02d:%02d", $f[3], $f[4], $f[5] + 1900, $f[2], $f[1], $f[0]),
        $err);
    ...
    if ($err)
    {
         push @{ $cont->{rows} }, 
         [
             format_string_state('ERROR: mysql status check has expired or is not available.',1,1)
         ];
         push @{ $cont->{class} }, [scalar @{ $cont->{rows} } + 2, 1, ''];
         push @{ $cont->{cellColSpans} }, [ $#{ $cont->{rows} } + 2, 1, 2 ];
    } 
